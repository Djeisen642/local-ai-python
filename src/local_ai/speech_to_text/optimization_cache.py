...
class OptimizationCache:
    ...
    def _get_system_fingerprint(self) -> str:
        ...
        try:
            ...
            if torch.cuda.is_available():
                ...
                system_data["gpu_memory"] = torch.cuda.get_device_properties(
                    0
                ).total_memory
        except ImportError:
            pass
    ...
    def cache_config(self, optimization_target: str, config: dict[str, Any]) -> None:
        ...
        try:
            ...
            cached_configs[cache_key] = {"config": config, "timestamp": time.time()}

            # Clean old entries (keep only last N per system)
            system_entries = [
                (k, v) for k, v in cached_configs.items() if k.startswith(fingerprint)
            ]
    ...
    def get_cached_performance_history(self) -> dict[str, Any] | None:
        """Get cached performance history."""
        if not self._is_cache_valid(
            self.performance_cache_file, self.performance_cache_ttl
        ):
            return None

        try:
            with open(self.performance_cache_file) as f:
                return json.load(f)
        ...
        return {
            ...
            "system_cache": {
                ...
                "valid": self._is_cache_valid(
                    self.system_cache_file, self.system_cache_ttl
                ),
                "size": self.system_cache_file.stat().st_size
                if self.system_cache_file.exists()
                else 0,
                "age_hours": (time.time() - self.system_cache_file.stat().st_mtime) / 3600
                if self.system_cache_file.exists()
                else 0,
            },
            "config_cache": {
                ...
                "valid": self._is_cache_valid(
                    self.config_cache_file, self.config_cache_ttl
                ),
                "size": self.config_cache_file.stat().st_size
                if self.config_cache_file.exists()
                else 0,
                "age_hours": (time.time() - self.config_cache_file.stat().st_mtime) / 3600
                if self.config_cache_file.exists()
                else 0,
            },
            "performance_cache": {
                ...
                "size": self.performance_cache_file.stat().st_size
                if self.performance_cache_file.exists()
                else 0,
                "age_hours": (time.time() - self.performance_cache_file.stat().st_mtime)
                / 3600
                if self.performance_cache_file.exists()
                else 0,
            },
...
_global_cache: OptimizationCache | None = None
