...
class PluginProcessingPipeline(ProcessingPipeline):
    ...
    def __init__(self) -> None:
        ...
        self._handlers: dict[ProcessingStage, dict[str, ProcessingHandler]] = defaultdict(
            dict
        )
    ...
    def register_handler(self, handler: ProcessingHandler) -> bool:
        ...
        try:
            ...
            if name in self._handlers[stage]:
                logger.warning(
                    f"Handler '{name}' already registered for '{stage.value}', replacing"
                )
    ...
    def get_registered_handlers(
        self, stage: ProcessingStage | None = None
    ) -> list[ProcessingHandler]:
        """
    ...
    async def process_transcription(
        ...
        async with self._processing_lock:
            ...
            try:
                # Process stages: EMBEDDING -> RESPONSE_GENERATION -> TEXT_TO_SPEECH
                processing_order = [
                ...
                logger.debug(f"Processed transcription through {len(results)} handlers")
    ...
    async def _process_stage(
        self, stage: ProcessingStage, context: ProcessingContext
    ) -> list[ProcessingResult]:
        """
        ...
        for name, handler in stage_handlers.items():
            if handler.can_handle(context):
                task = asyncio.create_task(
                    self._execute_handler(handler, context), name=f"{stage.value}_{name}"
                )
                tasks.append(task)
            else:
                logger.debug(
                    f"Handler '{name}' cannot handle context for stage '{stage.value}'"
                )
    ...
    async def _execute_handler(
        self, handler: ProcessingHandler, context: ProcessingContext
    ) -> ProcessingResult:
        """
        ...
        try:
            ...
            logger.debug(
                f"Handler '{handler.name}' completed in {result.processing_time:.3f}s"
            )
            return result
            ...
            processing_time = (datetime.now() - start_time).total_seconds()
            logger.error(
                f"Handler '{handler.name}' failed after {processing_time:.3f}s: {e}"
            )
