...
class VoiceActivityDetector:
    ...
    def __init__(self, sample_rate: int = None, frame_duration: int = None) -> None:
        ...
        if self.frame_duration not in VAD_SUPPORTED_FRAME_DURATIONS:
            raise ValueError(
                f"Unsupported frame duration: {self.frame_duration}. "
        ...
        logger.debug(
            f"🔊 VAD initialized: sample_rate={self.sample_rate}Hz, "
    ...
    def is_speech(self, audio_chunk: bytes) -> bool:
        ...
        if len(audio_chunk) != expected_bytes:
            ...
            if len(audio_chunk) < expected_bytes:
                logger.debug(
                    f"VAD chunk too short: {len(audio_chunk)} < {expected_bytes} bytes"
                )
                return False
            # If audio is too long, truncate to frame size
            logger.debug(
                f"VAD chunk too long: {len(audio_chunk)} > {expected_bytes}, truncating"
            )
        ...
        try:
            ...
            if current_time - self._last_debug_log >= self._debug_log_interval:
                speech_ratio = (
                    (self._speech_detections / self._total_chunks_processed * 100)
                    if self._total_chunks_processed > 0
                    else 0
                )
                logger.trace(
                    f"🔊 VAD Stats: {self._speech_detections} speech detections in "
    ...
    def detect_speech_end(self, current_time: float | None = None) -> bool:
        """
        ...
        if (
            self._speech_segment_start_time is not None
            and current_time - self._speech_segment_start_time >= MAX_SEGMENT_DURATION
        ):
            return True
        ...
        if silence_duration >= self._adaptive_long_pause:
            ...
            return True
        if silence_duration >= self._adaptive_medium_pause:
            # Likely sentence boundary - finalize transcription
            return True

        return False

    def reset_silence_timer(self, current_time: float | None = None) -> None:
        """
    ...
    def start_silence_timer(self, current_time: float | None = None) -> None:
        """
    ...
    def get_current_silence_duration(self, current_time: float | None = None) -> float:
        """
    ...
    def get_speech_segment_duration(self, current_time: float | None = None) -> float:
        """
    ...
    def _adapt_thresholds(self) -> None:
        ...
        target_medium = max(
            ADAPTIVE_PAUSE_MIN_MEDIUM, min(ADAPTIVE_PAUSE_MAX_MEDIUM, avg_pause * 1.2)
        )
        self._adaptive_medium_pause += (
            target_medium - self._adaptive_medium_pause
        ) * adaptation_rate

        # Adjust long pause threshold proportionally
        self._adaptive_long_pause = max(
            self._adaptive_medium_pause + 0.5, LONG_PAUSE_THRESHOLD
        )

        # Keep short pause threshold relatively stable but slightly adaptive
        target_short = max(
            ADAPTIVE_PAUSE_MIN_SHORT, min(ADAPTIVE_PAUSE_MAX_SHORT, avg_pause * 0.8)
        )
        self._adaptive_short_pause += (
            (target_short - self._adaptive_short_pause) * adaptation_rate * 0.5
        )
