...
class PerformanceMonitor:
    ...
    def start_operation(self, operation_id: str, operation_type: str) -> None:
        ...
        Args:
            ...
            operation_type: Type of operation (e.g., "transcription", "vad", "audio_capture")
        """
        self.start_times[operation_id] = time.time()
        self.operation_counters[operation_type] = self.operation_counters.get(operation_type, 0) + 1
    ...
    def get_operation_stats(self, operation_type: str, time_window: Optional[float] = None) -> Dict[str, Any]:
        """
    ...
    def get_overall_stats(self, time_window: Optional[float] = None) -> Dict[str, Any]:
        """
    ...
    def detect_performance_issues(self) -> List[Dict[str, Any]]:
        ...
        if transcription_stats["count"] > 0:
            ...
            if transcription_stats["avg_duration"] > 5.0:
                issues.append({
                    "type": "high_latency",
                    "severity": "warning",
                    "description": f"Average transcription latency is {transcription_stats['avg_duration']:.2f}s",
                    "recommendation": "Consider optimizing for latency or using a smaller model"
                })
            ...
            if transcription_stats["success_rate"] < 0.9:
                issues.append({
                    ...
                    "severity": "error",
                    "description": f"Transcription success rate is {transcription_stats['success_rate']:.1%}",
                    "recommendation": "Check for audio input issues or model availability"
                })
        ...
        if vad_stats["count"] > 0 and vad_stats["avg_duration"] > VAD_SLOW_THRESHOLD:
            issues.append({
                "type": "slow_vad",
                "severity": "info",
                "description": f"VAD processing is slower than expected: {vad_stats['avg_duration']:.3f}s",
                "recommendation": "Consider adjusting VAD frame duration or aggressiveness"
            })
    ...
    def get_performance_report(self) -> str:
        ...
        report.append(f"Total operations (last 5 min): {overall_stats['total_operations']}")
        report.append("")
...
class PerformanceContext:
    ...
    def __init__(self, operation_type: str, operation_id: Optional[str] = None, 
        ...
        self.operation_id = operation_id or f"{operation_type}_{int(time.time() * 1000000)}"
