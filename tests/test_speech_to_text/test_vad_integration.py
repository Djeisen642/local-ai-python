...
class TestVoiceActivityDetectorIntegration:
    ...
    def load_wav_file(self, file_path: Path) -> bytes:
        ...
        with wave.open(str(file_path), 'rb') as wav_file:
            ...
            if channels == 2:
                ...
                mono_data = bytearray()
                for i in range(0, len(audio_data), 4):  # 4 bytes = 2 samples * 2 bytes each
    ...
    def test_silence_detection(self, vad: VoiceActivityDetector, test_data_dir: Path) -> None:
        ...
        assert silence_ratio >= 0.8, f"Expected mostly silence, got {silence_ratio:.2%} silence"
        ...
        assert speech_ratio <= 0.2, f"Expected little speech in silence, got {speech_ratio:.2%} speech"
    
    @pytest.mark.integration
    def test_speech_detection(self, vad: VoiceActivityDetector, test_data_dir: Path) -> None:
        ...
        for filename in speech_files:
            ...
            assert speech_ratio >= 0.1, f"Expected some speech in {filename}, got {speech_ratio:.2%} speech"
    
    @pytest.mark.integration
    def test_mixed_audio_segmentation(self, vad: VoiceActivityDetector, test_data_dir: Path) -> None:
        ...
        assert len(speech_segments) <= len(chunks), "Cannot have more speech segments than total chunks"
        
        # All speech segments should be valid audio chunks
        for segment in speech_segments:
            assert isinstance(segment, bytes), "Each segment should be bytes"
            assert len(segment) == test_vad.frame_size * 2, "Each segment should be the correct size"
    ...
    def test_different_sample_rates(self, test_data_dir: Path) -> None:
        ...
        for filename, expected_rate in test_cases:
            ...
            if actual_rate != expected_rate:
                pytest.skip(f"Sample rate mismatch: expected {expected_rate}, got {actual_rate}")
                continue
            ...
            speech_ratio = len(speech_segments) / len(chunks)
            assert speech_ratio >= 0.1, f"Expected some speech detection at {expected_rate}Hz"
    
    @pytest.mark.integration
    def test_very_short_audio(self, vad: VoiceActivityDetector, test_data_dir: Path) -> None:
        ...
        # Basic sanity checks
        assert isinstance(speech_segments, list), "Should return a list even for short audio"
        assert len(speech_segments) <= len(chunks), "Cannot have more segments than chunks"
    
    @pytest.mark.integration
    def test_scenario_audio_files(self, vad: VoiceActivityDetector, test_data_dir: Path) -> None:
        ...
        for filename in scenario_files:
            ...
            assert speech_ratio >= 0.1, f"Expected speech detection in {filename}, got {speech_ratio:.2%}"
            
            # Log results for debugging
            print(f"{filename}: {len(speech_segments)}/{len(chunks)} chunks detected as speech ({speech_ratio:.1%})")