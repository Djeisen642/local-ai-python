...
from local_ai.speech_to_text.audio_capture import (
    AudioCapture,
    AudioCaptureError,
...
class TestEndToEndSpeechToTextPipeline:
    ...
    @pytest.mark.asyncio
    async def test_complete_pipeline_with_real_components(
        self, service: SpeechToTextService, test_audio_dir: Path
    ) -> None:
        """
        ...
        def capture_transcription(result: TranscriptionResult) -> None:
            transcription_results.append(result.text)

        service.set_transcription_result_callback(capture_transcription)
        ...
        class MockAudioCapture:
            ...
            def get_audio_chunk(self):
                ...
                chunk_end = min(
                    self.position + self.chunk_size * 2, len(self.audio_data)
                )  # 2 bytes per sample
        ...
        with patch.object(service, "_initialize_components") as mock_init:
            ...
            try:
                ...
                final_result = service.get_latest_transcription_result()
                assert final_result is not None, (
                    "Expected final transcription to be available"
                )
                final_transcription = final_result.text
                assert len(final_transcription.strip()) > 0, (
                ...
                found_expected = any(
                    word in transcription_lower for word in expected_words
                )
    ...
    @pytest.mark.integration
    @pytest.mark.asyncio
    async def test_pipeline_performance_and_latency(
        self, service: SpeechToTextService, test_audio_dir: Path
    ) -> None:
        """
        ...
        def capture_transcription_with_timing(result: TranscriptionResult) -> None:
            transcription_times.append(time.time())
            transcription_results.append(result.text)

        service.set_transcription_result_callback(capture_transcription_with_timing)
    ...
    async def test_error_scenarios_and_recovery(
        ...
        with patch(
            "local_ai.speech_to_text.audio_capture.pyaudio.PyAudio"
        ) as mock_pyaudio:
        ...
        with patch.object(
            AudioCapture,
            "start_capture",
            side_effect=MicrophoneNotFoundError("No microphone found"),
        ):
    ...
    @pytest.mark.integration
    @pytest.mark.asyncio
    async def test_concurrent_transcription_handling(
        self, service: SpeechToTextService, test_audio_dir: Path
    ) -> None:
        """
                ...
                for result in transcription_results:
                    ...
                    if result.strip():  # Only check non-empty results
                        assert len(result.strip()) > 0, (
                            "Non-empty results should have content"
                        )
    ...
    @pytest.mark.integration
    @pytest.mark.asyncio
    async def test_different_audio_scenarios(
        self, service: SpeechToTextService, test_audio_dir: Path
    ) -> None:
        """
        ...
        for filename, expected_words in scenarios:
            ...
            def scenario_callback(result: TranscriptionResult) -> None:
                transcription_results.append(result.text)

            service.set_transcription_result_callback(scenario_callback)
                ...
                try:
                    ...
                    if transcription_results:
                        ...
                        if final_result:
                            ...
                            if (
                                len(found_words) == 0
                                and len(final_transcription.strip()) > 0
                            ):
                                ...
                                assert (
                                    len(found_words) > 0
                                    or len(final_transcription.strip()) > 0
                                ), (
    ...
    @pytest.mark.integration
    @pytest.mark.asyncio
    async def test_service_lifecycle_robustness(
        self, service: SpeechToTextService
    ) -> None:
        """
                    ...
                    with patch.object(service, "_audio_capture") as mock_audio:
                        with patch.object(service, "_vad"):
                            with patch.object(
                                ...
                                await service.start_listening()
                                assert service.is_listening(), (
                                    f"Service should be listening in cycle {cycle}"
                                )
                                ...
                                await service.stop_listening()
                                assert not service.is_listening(), (
                                    f"Service should not be listening after stop in cycle {cycle}"
                                )
    ...
    async def test_memory_and_resource_management(
        ...
        def count_transcriptions(result: TranscriptionResult) -> None:
            nonlocal transcription_count
            transcription_count += 1

        service.set_transcription_result_callback(count_transcriptions)
        ...
        with patch.object(service, "_initialize_components") as mock_init:
            ...
            try:
                ...
                assert transcription_count >= 0, (
                    "Should have attempted transcription processing"
                )
...
class TestEndToEndErrorRecovery:
    ...
    @pytest.mark.integration
    @pytest.mark.asyncio
    async def test_audio_device_disconnection_recovery(
        self, service: SpeechToTextService
    ) -> None:
        """
    ...
    async def test_callback_exception_handling(
        ...
        def failing_callback(result: TranscriptionResult) -> None:
            nonlocal callback_call_count
            callback_call_count += 1
            raise Exception("Callback processing failed")

        service.set_transcription_result_callback(failing_callback)
        ...
        try:
            ...
            service._update_transcription_with_result(test_result)
        except Exception:
        ...
        final_result = service.get_latest_transcription_result()
        assert final_result is not None and final_result.text == "Test transcription"

        # Test with multiple failing callbacks
        for i in range(5):
            test_result = TranscriptionResult(
                text=f"Test {i}", confidence=0.95, timestamp=float(i), processing_time=0.1
            )
            service._update_transcription_with_result(test_result)
        ...
        final_result = service.get_latest_transcription_result()
        assert (
            final_result is not None and final_result.text == "Test 4"
        )  # Last transcription
