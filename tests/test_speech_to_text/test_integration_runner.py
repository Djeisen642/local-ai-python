...
class IntegrationTestRunner:
    ...
    def run_integration_tests(self, verbose: bool = True) -> Dict[str, Any]:
        ...
        if verbose:
            print("Starting Speech-to-Text Integration Test Suite")
            print("=" * 60)
        ...
        for category, test_file in test_categories.items():
            if verbose:
                print(f"\nRunning {category} tests from {test_file}...")
    ...
    def _parse_pytest_json_report(self) -> Optional[Dict[str, Any]]:
        """Parse pytest JSON report if available."""
        try:
    ...
    def _print_summary(self) -> None:
        ...
        for category, result in self.results.items():
            ...
            print(f"{category:25} {status:6} ({duration:.2f}s)")
    ...
    def _analyze_requirements_coverage(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "coverage_percentage": len(covered_requirements) / len(target_requirements) * 100,
            "missing_requirements": [req for req in target_requirements if req not in covered_requirements]
        }
...
def main():
    ...
    parser.add_argument("--category", choices=["end_to_end", "performance", "transcriber", "vad"],
    ...
    try:
        if args.category:
            report = runner.run_specific_test_category(args.category, verbose=not args.quiet)
        else:
