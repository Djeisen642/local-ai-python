...
class MockFailingHandler(ProcessingHandler):
    ...
    def __init__(
        self,
        name: str = "failing_handler",
        stage: ProcessingStage = ProcessingStage.EMBEDDING,
    ) -> None:
        self._name = name
        self._stage = stage
...
class TestPluginProcessingPipeline:
    ...
    def test_handler_unregistration(self) -> None:
        ...
        # Unregister handler
        assert (
            pipeline.unregister_handler(ProcessingStage.EMBEDDING, "test_handler") is True
        )
        ...
        assert (
            pipeline.unregister_handler(ProcessingStage.EMBEDDING, "nonexistent") is False
        )
    ...
    async def test_pipeline_processing_success(self) -> None:
        ...
        embedding_result = next(
            r for r in results if r.stage == ProcessingStage.EMBEDDING
        )
        response_result = next(
            r for r in results if r.stage == ProcessingStage.RESPONSE_GENERATION
        )
    ...
    async def test_pipeline_processing_with_failures(self) -> None:
        ...
        )  # Pipeline succeeded despite handler failure
        assert stats["failed_processed"] == 0
    ...
    async def test_pipeline_concurrent_processing(self) -> None:
        ...
        )  # Should be much less than 0.3 (3 * 0.1) if concurrent
