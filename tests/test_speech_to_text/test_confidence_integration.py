...
class TestConfidenceRatingIntegration:
    ...
    async def _transcribe_file_with_confidence(self, transcriber: WhisperTranscriber, file_path: Path) -> TranscriptionResult:
    ...
    @pytest.mark.asyncio
    async def test_confidence_calculation_with_high_quality_audio(self, transcriber: WhisperTranscriber) -> None:
        """
        ...
        assert 0.0 <= result.confidence <= 1.0, f"Confidence should be between 0.0 and 1.0, got: {result.confidence}"
        ...
        if result.text.strip():  # Only check if we got actual transcription
            assert result.confidence >= 0.0, "High quality audio should have non-negative confidence"
    ...
    @pytest.mark.integration
    @pytest.mark.asyncio
    async def test_confidence_calculation_with_low_quality_audio(self, transcriber: WhisperTranscriber) -> None:
        """
    ...
    @pytest.mark.integration
    @pytest.mark.asyncio
    async def test_confidence_calculation_with_silence(self, transcriber: WhisperTranscriber) -> None:
        """
    ...
    @pytest.mark.integration
    @pytest.mark.asyncio
    async def test_confidence_calculation_with_clear_speech(self, transcriber: WhisperTranscriber) -> None:
        """
        ...
        for filename in test_files:
            ...
            assert isinstance(result, TranscriptionResult)
            assert 0.0 <= result.confidence <= 1.0, f"Confidence should be between 0.0 and 1.0 for {filename}, got: {result.confidence}"
            ...
            if result.text.strip():  # Only collect scores for non-empty transcriptions
                ...
                assert result.confidence >= 0.0, f"Clear speech should have non-negative confidence for {filename}"
        
        # Verify we got some confidence scores
        assert len(confidence_scores) > 0, "Should have calculated confidence for at least one clear speech file"
        ...
        if len(confidence_scores) > 1:
            ...
            assert len(unique_scores) >= 1, "Should have at least one unique confidence score"

    @pytest.mark.integration
    @pytest.mark.asyncio
    async def test_confidence_data_flow_through_service_callbacks(self, service: SpeechToTextService) -> None:
        """
        ...
        def text_callback(text: str) -> None:
            """Callback to capture text transcriptions."""
            text_results.append(text)
        
        # Set up callbacks
        service.set_transcription_result_callback(result_callback)
        service.set_transcription_callback(text_callback)
        ...
        with patch.object(service, '_initialize_components') as mock_init:
            ...
            try:
                ...
                for result in transcription_results:
                    assert isinstance(result, TranscriptionResult), "Callback should receive TranscriptionResult objects"
                    assert hasattr(result, 'confidence'), "TranscriptionResult should have confidence attribute"
                    assert 0.0 <= result.confidence <= 1.0, f"Confidence should be between 0.0 and 1.0, got: {result.confidence}"
                    assert hasattr(result, 'text'), "TranscriptionResult should have text attribute"
                    assert hasattr(result, 'timestamp'), "TranscriptionResult should have timestamp attribute"
                    assert hasattr(result, 'processing_time'), "TranscriptionResult should have processing_time attribute"
                ...
                # Verify that both callback types can coexist
                # Text callback should still work alongside result callback
                if len(text_results) > 0 and len(transcription_results) > 0:
                    # If both callbacks were triggered, verify consistency
                    assert len(text_results) <= len(transcription_results) + 1, "Text and result callbacks should be roughly consistent"
    ...
    @pytest.mark.integration
    @pytest.mark.asyncio
    async def test_confidence_accuracy_across_audio_qualities(self, transcriber: WhisperTranscriber) -> None:
        """
        ...
        for filename, quality_type in quality_tests:
            ...
            assert isinstance(result, TranscriptionResult)
            assert 0.0 <= result.confidence <= 1.0, f"Confidence should be valid for {quality_type}, got: {result.confidence}"
            assert isinstance(result.text, str)
            assert result.processing_time > 0.0
            assert result.timestamp > 0.0
        ...
        if "high_quality" in results and "silence" in results:
            ...
            print(f"Confidence comparison - High quality: {high_quality_conf:.3f}, Silence: {silence_conf:.3f}")
    ...
    @pytest.mark.integration
    @pytest.mark.asyncio
    async def test_confidence_calculation_consistency(self, transcriber: WhisperTranscriber) -> None:
        """
        ...
        if len(texts) > 1:
            ...
            assert len(set(texts)) <= len(texts), "Should have some consistent transcriptions"

    @pytest.mark.integration
    @pytest.mark.asyncio
    async def test_confidence_with_different_speech_scenarios(self, transcriber: WhisperTranscriber) -> None:
        """
        ...
        for filename, scenario_type in scenarios:
            ...
            assert isinstance(result, TranscriptionResult)
            assert 0.0 <= result.confidence <= 1.0, f"Confidence should be valid for {scenario_type}, got: {result.confidence}"
        ...
        assert len(scenario_results) > 0, "Should have tested at least one speech scenario"
        
        # Log confidence scores for different scenarios (informational)
        for scenario_type, result in scenario_results.items():
            print(f"Scenario '{scenario_type}': confidence={result.confidence:.3f}, text_length={len(result.text)}")

    @pytest.mark.integration
    @pytest.mark.asyncio
    async def test_confidence_bounds_validation(self, transcriber: WhisperTranscriber) -> None:
        """
        ...
        for filename in test_files:
            ...
            assert 0.0 <= result.confidence <= 1.0, f"Confidence out of bounds for {filename}: {result.confidence}"
            ...
            assert isinstance(result.confidence, float), f"Confidence should be float for {filename}"
        
        # Verify we got some confidence scores
        assert len(all_confidences) > 0, "Should have calculated confidence for at least one file"
        
        # Verify the range of confidence scores makes sense
        min_confidence = min(all_confidences)
        max_confidence = max(all_confidences)
        
        assert 0.0 <= min_confidence <= 1.0, f"Minimum confidence out of bounds: {min_confidence}"
        assert 0.0 <= max_confidence <= 1.0, f"Maximum confidence out of bounds: {max_confidence}"
        assert min_confidence <= max_confidence, "Min confidence should be <= max confidence"
        
        # Log confidence range for analysis
        print(f"Confidence range across all test files: {min_confidence:.3f} to {max_confidence:.3f}")