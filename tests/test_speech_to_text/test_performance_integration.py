...
class TestSpeechToTextPerformance:
    ...
    @pytest.mark.asyncio
    async def test_transcription_latency_requirements(
        ...
        def measure_latency_callback(result: TranscriptionResult) -> None:
            transcription_results.append({"text": result.text, "timestamp": time.time()})

        service.set_transcription_result_callback(measure_latency_callback)
    ...
    async def test_throughput_performance(
        ...
        def throughput_callback(result: TranscriptionResult) -> None:
            nonlocal transcription_count
            transcription_count += 1

        service.set_transcription_result_callback(throughput_callback)
        ...
        with patch.object(service, "_initialize_components") as mock_init:
            ...
            try:
                ...
                assert total_time < 30.0, (
                    f"Total processing time too long: {total_time:.2f}s"
                )
                ...
                if transcription_count > 0:
                    ...
                    assert throughput > 0.1, (
                        f"Throughput too low: {throughput:.2f} transcriptions/second"
                    )
    ...
    async def test_memory_usage_under_load(
        ...
        def memory_callback(result: TranscriptionResult) -> None:
            nonlocal transcription_count
            transcription_count += 1
            # Sample memory usage
        ...
        service.set_transcription_result_callback(memory_callback)
            ...
            try:
                ...
                if len(memory_samples) > 5:
                    ...
                    assert growth_rate < 1.0, (
                        f"Memory leak detected: {growth_rate:.2f}MB per transcription"
                    )
    ...
    @pytest.mark.performance
    @pytest.mark.asyncio
    async def test_cpu_usage_efficiency(
        self, service: SpeechToTextService, test_audio_dir: Path
    ) -> None:
        """
        ...
        def cpu_callback(result: TranscriptionResult) -> None:
            nonlocal transcription_count
            transcription_count += 1

        service.set_transcription_result_callback(cpu_callback)
        ...
        if cpu_samples:
            ...
            # CPU usage should be reasonable (< 90% average, < 98% peak)
            # Adjusted thresholds for test environment variability
            assert avg_cpu < 90.0, f"Average CPU usage too high: {avg_cpu:.1f}%"
            assert max_cpu < 98.0, f"Peak CPU usage too high: {max_cpu:.1f}%"
    ...
    async def test_concurrent_processing_performance(
        ...
        def concurrent_callback(result: TranscriptionResult) -> None:
            transcription_results.append({"text": result.text, "timestamp": time.time()})

        service.set_transcription_result_callback(concurrent_callback)
            ...
            def get_audio_chunk(self):
                ...
                while attempts < len(self.audio_list):
                    ...
                    if position < len(current_audio):
                        chunk_end = min(
                            position + self.chunk_size * 2, len(current_audio)
                        )
                        ...
                        self.current_audio_index = (self.current_audio_index + 1) % len(
                            self.audio_list
                        )
                ...
                if transcription_results:
                    ...
                    if len(transcription_results) > 1:
                        ...
                        for i in range(1, len(transcription_results)):
                            interval = (
                                transcription_results[i]["timestamp"]
                                - transcription_results[i - 1]["timestamp"]
                            )
                            intervals.append(interval)

                        avg_interval = statistics.mean(intervals)
                        assert avg_interval < 5.0, (
                            f"Average processing interval too long: {avg_interval:.2f}s"
                        )
    ...
    @pytest.mark.performance
    @pytest.mark.asyncio
    async def test_audio_quality_impact_on_performance(
        self, service: SpeechToTextService, test_audio_dir: Path
    ) -> None:
        """
        ...
        for filename, quality_label in quality_tests:
            ...
            def quality_callback(result: TranscriptionResult) -> None:
                transcription_results.append(
                    {"text": result.text, "timestamp": time.time()}
                )

            service.set_transcription_result_callback(quality_callback)
        ...
        if len(quality_results) > 1:
            processing_times = [
                result["processing_time"] for result in quality_results.values()
            ]
