...
class DemoEmbeddingHandler(EmbeddingHandler):
    ...
    async def generate_embedding(self, text: str, metadata: Dict[str, Any]) -> List[float]:
        ...
        embedding = [
            ...
            1.0 if any(word in text.lower() for word in ["help", "please", "thank"]) else 0.0,  # Politeness
    ...
    def search_similar(self, query_embedding: List[float], top_k: int = 5) -> List[Dict[str, Any]]:
        ...
        return [{"id": emb_id, "similarity": sim, "data": data} 
...
class DemoResponseHandler(ResponseGenerationHandler):
    ...
    async def process(self, context: ProcessingContext) -> ProcessingResult:
        ...
        try:
            ...
            return ProcessingResult(
                ...
                data={
                    "response": response,
                    "conversation_turn": len(self.conversation_history.get(context.session_id, [])),
    ...
    def can_handle(self, context: ProcessingContext) -> bool:
        ...
        return ("?" in context.text or 
                any(word in text_lower for word in ["help", "what", "how", "why", "when", "where"]))
    ...
    async def generate_response(self, text: str, context: ProcessingContext) -> str:
        ...
        history = await self.get_conversation_context(context.session_id)
        
        # Search for similar past interactions
        if self.embedding_handler.embeddings_store:
            query_embedding = await self.embedding_handler.generate_embedding(text, context.metadata)
...
class TestExtensibilityDemo:
        ...
        for i, (user_input, results) in enumerate(zip(interactions, pipeline_results)):
            ...
            for result in results:
                if result.stage == ProcessingStage.EMBEDDING:
                    ...
                    print(f"  📊 Embedding: Generated {len(embedding_data['embedding_vector'])}-dim vector")
                
                elif result.stage == ProcessingStage.RESPONSE_GENERATION:
                    response_data = result.data
                    print(f"  🤖 Response: '{response_data['response']}'")
                    print(f"      Type: {response_data['response_type']}")
                
                elif result.stage == ProcessingStage.TEXT_TO_SPEECH:
                    tts_data = result.data
                    print(f"  🔊 TTS: Generated {tts_data['audio_size']} bytes of audio")
        
        print(f"\n=== Final Statistics ===")
        print(f"Total embeddings stored: {len(embedding_handler.embeddings_store)}")
        print(f"Conversation turns: {len(conversation)}")
        print(f"TTS syntheses: {len(synthesis_history)}")
    ...
    async def test_handler_filtering_and_selective_processing(self) -> None:
        ...
        # Register handlers
        embedding_handler = DemoEmbeddingHandler()
        response_handler = DemoResponseHandler(embedding_handler)  # Only handles questions
        tts_handler = DemoTTSHandler()
    ...
    def test_handler_registration_and_management(self) -> None:
        ...
        # Unregister handlers
        assert service.unregister_processing_handler("embedding", "demo_semantic_embeddings") is True
        assert service.unregister_processing_handler("response_generation", "demo_ai_response") is True
        assert service.unregister_processing_handler("text_to_speech", "demo_neural_tts") is True
