...
class TestArgumentHandling:
    ...
    def test_handle_arguments_reset_model_cache_success(self) -> None:
        ...
        with patch('logging.basicConfig'):
            with patch('local_ai.main.reset_model_cache', return_value=True) as mock_reset:
    ...
    def test_handle_arguments_reset_optimization_cache_success(self) -> None:
        ...
        with patch('logging.basicConfig'):
            with patch('local_ai.main.reset_optimization_cache', return_value=True) as mock_reset:
                with patch('builtins.print') as mock_print:
                    success, should_continue = handle_arguments(args)
    ...
    def test_handle_arguments_reset_model_cache_failure(self) -> None:
        ...
        with patch('logging.basicConfig'):
            with patch('local_ai.main.reset_model_cache', return_value=False) as mock_reset:
                with patch('builtins.print') as mock_print:
                    success, should_continue = handle_arguments(args)
    ...
    def test_handle_arguments_reset_optimization_cache_failure(self) -> None:
        ...
        with patch('logging.basicConfig'):
            with patch('local_ai.main.reset_optimization_cache', return_value=False) as mock_reset:
                with patch('builtins.print') as mock_print:
                    success, should_continue = handle_arguments(args)
    ...
    def test_handle_arguments_reset_model_cache_exception(self) -> None:
        ...
        with patch('logging.basicConfig'):
            with patch('local_ai.main.reset_model_cache', side_effect=Exception("Cache error")) as mock_reset:
                with patch('builtins.print') as mock_print:
                    success, should_continue = handle_arguments(args)
    ...
    def test_handle_arguments_reset_optimization_cache_exception(self) -> None:
        ...
        with patch('logging.basicConfig'):
            with patch('local_ai.main.reset_optimization_cache', side_effect=Exception("Opt cache error")) as mock_reset:
                with patch('builtins.print') as mock_print:
                    success, should_continue = handle_arguments(args)
        
        mock_reset.assert_called_once()
        mock_print.assert_any_call("❌ Error clearing optimization cache: Opt cache error")
        assert success is False
        assert should_continue is False
    ...
    def test_handle_arguments_both_caches(self) -> None:
        ...
        with patch('logging.basicConfig'):
            with patch('local_ai.main.reset_model_cache', return_value=True) as mock_reset_model:
                with patch('local_ai.main.reset_optimization_cache', return_value=True) as mock_reset_opt:
                    with patch('builtins.print') as mock_print:
                        success, should_continue = handle_arguments(args)
...
class TestCLIIntegration:
    ...
    def test_cli_entry_with_reset_model_cache(self) -> None:
        ...
        with patch('sys.argv', ['main.py'] + test_args):
            with patch('local_ai.main.handle_arguments', return_value=(True, False)) as mock_handle:
    ...
    def test_cli_entry_with_failed_cache_reset(self) -> None:
        ...
        with patch('sys.argv', ['main.py'] + test_args):
            with patch('local_ai.main.handle_arguments', return_value=(False, False)) as mock_handle:
                with patch('local_ai.main.main') as mock_main:
                    with patch('asyncio.run') as mock_asyncio_run:
                        with pytest.raises(SystemExit) as exc_info:
                            from local_ai.main import cli_entry_with_args
